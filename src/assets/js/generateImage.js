...
function disconnect(afterClose) {
...
function tabletDemo(afterClose) {
  var p = new WacomGSS.STU.Protocol();
  var intf;
...
function addButtons(afterClose) {
  m_btns = new Array(3);
  m_btns[0] = new Button();
  m_btns[1] = new Button();
  m_btns[2] = new Button();
  ...
  m_btns[0].Click = function () { btnOk_Click(afterClose); };
...
function btnOk_Click(afterClose) {
  // You probably want to add additional processing here.
  generateImage();
  setTimeout(function (){close(afterClose)}, 0);
}
